...
import { supabaseConfigured, supabaseRest, supabaseUpsert, supabasePatch, supabaseDelete } from './supabase';
...
export async function syncAccountToSupabase(user: User): Promise<void> {
...
export async function hydrateAccountFromSupabase(userId: string): Promise<User | null> {
  if (!supabaseConfigured) return null;
  try {
    const rows = await supabaseRest<any[]>(`profiles?id=eq.${encodeURIComponent(userId)}&select=*`);
    const row = rows[0];
    if (!row) return null;
    const user: User = {
      id: row.id,
      ...
      isAdmin: !!row.is_admin || isUserAdmin(row.username),
      createdAt: new Date(row.created_at).getTime(),
    };
    const accounts = getStoredAccounts();
    const idx = accounts.findIndex(a => a.id === user.id);
    const account: RobloxAccount = {
      id: user.id, robloxId: Number(user.robloxId), username: user.username,
      displayName: user.displayName || user.username, avatar: user.avatar,
      balance: user.balance, level: user.level, isAdmin: user.isAdmin,
      createdAt: user.createdAt, lastLoginAt: Date.now(),
    };
    ...
    return user;
  } catch (err) { console.warn('Supabase profile hydrate failed:', err); return null; }
}

export async function syncPlayerPetsToSupabase(username: string): Promise<void> {
  if (!supabaseConfigured || !username) return;
  try {
    await supabaseDelete('player_pets', `username=ilike.${encodeURIComponent(username)}`);
    const pets = getAllPlayerInventories()[username.trim().toLowerCase()] || [];
    if (pets.length) await supabaseUpsert('player_pets', pets.map(p => ({
      id: p.id, username, pet_id: p.petId, name: p.name, image_url: p.imageUrl,
      value_in_robux: p.valueInRobux, rarity: p.rarity || null, assigned_at: p.assignedAt,
    })));
  } catch (err) { console.warn('Supabase inventory sync failed:', err); }
}

export async function hydratePlayerPetsFromSupabase(username: string): Promise<PlayerPetItem[]> {
  if (!supabaseConfigured || !username) return getPlayerPets(username);
  try {
    const rows = await supabaseRest<any[]>(`player_pets?username=ilike.${encodeURIComponent(username)}&select=*`);
    const pets = rows.map(row => ({ id: row.id, petId: row.pet_id, name: row.name,
      imageUrl: row.image_url, valueInRobux: Number(row.value_in_robux || 0), rarity: row.rarity || undefined,
      assignedAt: Number(row.assigned_at || Date.now()) }));
    const inventories = getAllPlayerInventories();
    inventories[username.trim().toLowerCase()] = pets;
    localStorage.setItem(STORAGE_KEY_PLAYER_PETS, JSON.stringify(inventories));
    return pets;
  } catch (err) { console.warn('Supabase inventory hydrate failed:', err); return getPlayerPets(username); }
}

export async function fetchGamesFromSupabase(): Promise<CoinflipGame[]> {
  if (!supabaseConfigured) return [];
  try {
    const rows = await supabaseRest<any[]>('coinflip_games?status=in.(waiting,active)&order=created_at.desc&select=*');
    return rows.map(row => ({
      id: row.id, creator: row.creator, challenger: row.challenger || undefined,
      creatorSide: row.creator_side, betAmount: Number(row.bet_amount || 0),
      betType: row.bet_type, creatorPets: row.creator_pets || undefined, challengerPets: row.challenger_pets || undefined,
      status: row.status, createdAt: Number(row.created_at), winner: row.winner || undefined,
      winningSide: row.winning_side || undefined, serverSeedHash: row.server_seed_hash || undefined,
      serverSeed: row.server_seed || undefined, clientSeed: row.client_seed || undefined,
      isBotMatch: !!row.is_bot_match,
    }));
  } catch (err) { console.warn('Supabase games fetch failed:', err); return []; }
}

export async function saveGameToSupabase(game: CoinflipGame): Promise<void> {
  if (!supabaseConfigured) return;
  try { await supabaseUpsert('coinflip_games', {
    id: game.id, creator_id: game.creator.id, challenger_id: game.challenger?.id || null,
    creator: game.creator, challenger: game.challenger || null, creator_side: game.creatorSide,
    bet_amount: game.betAmount, bet_type: game.betType || 'currency', creator_pets: game.creatorPets || null,
    challenger_pets: game.challengerPets || null, status: game.status, created_at: game.createdAt,
    winner: game.winner || null, winning_side: game.winningSide || null, server_seed_hash: game.serverSeedHash || null,
    server_seed: game.serverSeed || null, client_seed: game.clientSeed || null, is_bot_match: !!game.isBotMatch,
  }); } catch (err) { console.warn('Supabase game sync failed:', err); }
}

export async function deleteGameFromSupabase(gameId: string): Promise<void> {
  if (!supabaseConfigured) return;
  try { await supabaseDelete('coinflip_games', `id=eq.${encodeURIComponent(gameId)}`); }
  catch (err) { console.warn('Supabase game delete failed:', err); }
}
...
export function completeRobloxLogin(verifiedUser: {
  ...
  createSession(user);
  void syncAccountToSupabase(user);
  void hydratePlayerPetsFromSupabase(user.username);
  return user;
}
...
export function updateUserBalance(userId: string, newBalance: number): void {
  ...
  if (idx !== -1) {
    ...
    saveAccounts(accounts);
    const updatedAccount = accounts[idx];
    if (supabaseConfigured) void supabasePatch('profiles', `id=eq.${encodeURIComponent(userId)}`, { balance: updatedAccount.balance, level: updatedAccount.level, last_login_at: new Date().toISOString() }).catch(err => console.warn('Supabase balance sync failed:', err));
  }
...
export function addPetToPlayer(username: string, pet: PetValue): PlayerPetItem {
  ...
  void syncPlayerPetsToSupabase(username);
...
export function recordUserWager(
  ...
  try {
    ...
    if (supabaseConfigured) void supabaseUpsert('user_wagers', { username: map[key].username, roblox_id: Number(map[key].robloxId), avatar: map[key].avatar, wagered: map[key].wagered, updated_at: new Date().toISOString() }).catch(err => console.warn('Supabase wager sync failed:', err));
  } catch (err) {
